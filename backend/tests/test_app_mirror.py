...
class AppMirrorEndpointsTestCase(unittest.TestCase):
    def test_mirror_handlers(self):
        with patch("backend.src.app.mirror_service.status", return_value={"enabled": False}), patch("backend.src.app.mirror_service.enable_mirror", return_value={"enabled": True}), patch(
            ...
            self.assertFalse(mirror_status()["enabled"])
